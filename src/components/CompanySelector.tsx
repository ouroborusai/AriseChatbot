...
export default function CompanySelector({ className = '', variant = 'sidebar' }: CompanySelectorProps) {
    ...
    return (
      <div className={`relative ${className}`} ref={dropdownRef}>
        ...
        >
          ...
          <ChevronDown size={14} className={`text-slate-500 transition-transform duration-500 ${isDropdownOpen ? 'rotate-180 text-green-500' : ''}`} />
        </button>
        ...
        {isDropdownOpen && (
          <div className="absolute top-[calc(100%+12px)] right-0 w-80 bg-white border border-slate-200 rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] z-[200] p-4 animate-in fade-in zoom-in-95 duration-300 backdrop-blur-3xl">
             <div className="relative mb-4 group/search">
               <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/search:text-green-500 transition-colors" />
               <input 
                autoFocus
                type="text" 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 pl-12 pr-4 text-[10px] font-black text-slate-900 outline-none focus:border-green-500/30 transition-all placeholder:text-slate-400 focus:bg-white"
               />
            </div>
            <div className="max-h-[60vh] overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
              <div className="px-4 py-3 border-b border-slate-100 mb-3 flex justify-between items-center">
                 ...
                 <span className="text-[9px] font-black bg-slate-50 text-green-600 px-3 py-1 rounded-lg border border-slate-200">{filteredCompanies.length}</span>
              </div>
              {filteredCompanies.map((c) => (
                ...
                >
                  <div className="flex flex-col gap-1 truncate pr-4 relative z-10">
                    ...
                    <span className={`text-[8px] font-black uppercase tracking-widest ${activeCompany?.id === c.id ? 'text-white/60' : 'text-green-600/70'}`}>
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      <button 
        onClick={() => setIsDropdownOpen(!isDropdownOpen)}
        className="w-full flex items-center justify-between gap-4 bg-transparent hover:text-green-600 transition-all py-2.5 text-[11px] font-black text-slate-900 outline-none group relative"
      >
        ...
        <ChevronDown size={16} className={`text-slate-400 transition-transform duration-500 ${isDropdownOpen ? 'rotate-180 text-green-500' : ''}`} />
      </button>
      ...
      {isDropdownOpen && (
        <div className="absolute top-[calc(100%+12px)] left-0 w-full bg-white border border-slate-100 rounded-2xl shadow-xl z-[100] p-3 animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="relative mb-3 group/search">
             <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/search:text-green-500 transition-colors" />
             <input 
              autoFocus
              type="text" 
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-slate-50 border border-slate-100 rounded-xl py-2.5 pl-10 pr-4 text-[10px] font-black text-slate-900 outline-none focus:border-green-500/30 transition-all placeholder:text-slate-400 focus:bg-white"
             />
          </div>
            ...
            {filteredCompanies.length > 0 ? (
              filteredCompanies.map((c) => (
                ...
                >
                  <div className="flex flex-col gap-1 truncate pr-4 relative z-10">
                    <span className="truncate uppercase tracking-tight">{c.name}</span>
                    <span className={`text-[8px] font-black uppercase tracking-widest ${activeCompany?.id === c.id ? 'text-slate-400' : 'text-green-600/70'}`}>
                      TIER_{c.plan_tier || 'FREE'}
                    </span>
                  </div>
                  {activeCompany?.id === c.id && <Check size={14} className="relative z-10" />}
