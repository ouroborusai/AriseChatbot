...
function AuthContent() {
  ...
  return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6 lg:p-10 font-sans selection:bg-green-500/30 selection:text-white text-slate-900 overflow-hidden relative">
      
      {/* PERFORMANCE: OPTIMIZED BACKGROUND WITH MESH GRADIENT - LUMINOUS VIBRANT */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
         {/* BASE SOLID GRADIENT (matches user reference image) */}
         ...
         <Image 
          ...
          fill
          priority
          className="object-cover opacity-100 scale-105 blur-xl" 
         />
      ...
      <div className="w-full max-w-[1200px] bg-white/60 backdrop-blur-3xl rounded-[48px] lg:rounded-[64px] shadow-[0_50px_150px_-20px_rgba(0,0,0,0.1)] flex flex-col lg:flex-row overflow-hidden relative border border-white z-10 group">
        ...
        <div className="hidden lg:flex lg:w-[45%] bg-slate-50 relative p-16 flex-col items-center justify-between overflow-hidden border-r border-slate-100">
          ...
          <div className="relative z-10 w-full flex items-center gap-4 animate-in fade-in slide-in-from-left-4 duration-700">
             ...
             <span className="text-[10px] font-black text-green-600 uppercase tracking-[0.5em] italic">Protocolo Luminous v2.5</span>
          </div>

          <div className="relative z-20 flex flex-col items-center justify-center w-full">
             <div className="relative group mb-12">
                <div className="absolute inset-0 bg-green-500/20 blur-[80px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
                <div className="w-48 h-48 bg-white/5 backdrop-blur-md rounded-[40px] border border-white/10 shadow-2xl flex items-center justify-center p-0 overflow-hidden relative group-hover:scale-105 transition-transform duration-700">
                   <Image src="/brand/official.png" alt="LOOP logo" fill className="object-cover" />
                </div>
             </div>

             <div className="flex flex-col items-center text-center">
                <h2 className="text-5xl font-black text-slate-900 leading-[0.9] tracking-tighter uppercase italic">
                  {view === 'login' && <>Acceso <br /><span className="text-green-600">Autorizado.</span></>}
                  {view === 'register' && <>Nuevo <br /><span className="text-green-600">Operador.</span></>}
                  {view === 'forgot' && <>Reseteo <br /><span className="text-green-600">Neural.</span></>}
                </h2>
        ...
        <div className="w-full lg:w-[55%] h-full bg-transparent relative overflow-hidden flex flex-col">
          ...
          <div className="flex-1 flex flex-col items-center px-8 lg:px-20 py-16 lg:py-24">
            ...
            {/* Header Section */}
            <div className="flex flex-col items-center mb-16 w-full animate-in fade-in slide-in-from-top-4 duration-700">
               ...
               <h1 className="text-3xl lg:text-4xl font-black text-slate-900 mb-3 tracking-tighter italic uppercase text-center">
            ...
            <div className="w-full max-w-[420px] mb-12 animate-in fade-in duration-1000">
              <div className="flex items-center bg-slate-100 p-1.5 rounded-[24px] border border-slate-200 shadow-inner">
                {(['login', 'register', 'forgot'] as AuthView[]).map((v) => (
                  <button 
                    key={v}
                    onClick={() => changeView(v)}
                    className={`flex-1 py-4 text-[9px] font-black uppercase tracking-widest text-center rounded-[18px] transition-all duration-500 ${
               ...
               <div className="animate-in fade-in duration-700">
                  {success ? (
                    <div className="text-center py-12 px-10 bg-green-500/5 border border-green-500/20 rounded-[40px] shadow-2xl">
                      <div className="w-16 h-16 bg-green-500/10 border border-green-500/20 rounded-full flex items-center justify-center mx-auto mb-8 text-green-500 shadow-2xl"><ShieldCheck size={32} /></div>
                      <h4 className="text-sm font-black text-white uppercase tracking-[0.2em] mb-4 italic">Solicitud Procesada</h4>
                      ...
                      <button onClick={() => changeView('login')} className="mt-10 text-[9px] font-black uppercase tracking-[0.3em] text-green-500 hover:text-white transition-colors">Volver al Terminal</button>
                    </div>
                  ) : (
                    <form onSubmit={view === 'login' ? handleLogin : view === 'register' ? handleRegister : handleReset} className="space-y-4">
                      {view === 'register' && (
                        <div className="relative group bg-slate-50 border border-slate-200 rounded-[22px] flex items-center overflow-hidden h-14 focus-within:border-green-600/30 focus-within:bg-white transition-all shadow-sm">
                           ...
                           <User size={16} className="ml-5 text-slate-300 group-focus-within:text-green-600 transition-colors shrink-0" />
                           <input required type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="NOMBRE COMPLETO"
                            className="flex-1 bg-transparent border-none px-5 h-full text-[11px] font-black text-slate-900 uppercase tracking-widest outline-none placeholder:text-slate-200" />
                        </div>
                      )}
                      ...
                      <div className="relative group bg-slate-50 border border-slate-200 rounded-[22px] flex items-center overflow-hidden h-14 focus-within:border-green-600/30 focus-within:bg-white transition-all shadow-sm">
                         ...
                         <Mail size={16} className="ml-5 text-slate-300 group-focus-within:text-green-600 transition-colors shrink-0" />
                      ...
                      {view !== 'forgot' && (
                        <>
                           <div className="relative group bg-slate-50 border border-slate-200 rounded-[22px] flex items-center overflow-hidden h-14 focus-within:border-green-600/30 focus-within:bg-white transition-all shadow-sm">
                              <div className="w-28 pl-6 flex items-center border-r border-slate-100 shrink-0">
                                 <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Clave</span>
                              </div>
                              <Lock size={16} className="ml-5 text-slate-300 group-focus-within:text-green-600 transition-colors shrink-0" />
                              <input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••"
                               className="flex-1 bg-transparent border-none px-5 h-full text-[11px] font-black text-slate-900 tracking-[0.3em] outline-none placeholder:text-slate-200" />
                           </div>
                           
                           {view === 'register' && (
                             <div className="relative group bg-slate-50 border border-slate-200 rounded-[22px] flex items-center overflow-hidden h-14 focus-within:border-green-600/30 focus-within:bg-white transition-all shadow-sm">
                                <div className="w-28 pl-6 flex items-center border-r border-slate-100 shrink-0">
                                   <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Confirmar</span>
                                </div>
                                <Lock size={16} className="ml-5 text-slate-300 group-focus-within:text-green-600 transition-colors shrink-0" />
                      ...
                      {view === 'login' && (
                        <div className="flex justify-end pr-2 pt-1">
                           <button onClick={() => changeView('forgot')} type="button" className="text-[9px] font-black text-slate-600 hover:text-green-500 uppercase tracking-widest transition-colors italic">¿Olvidaste tu clave?</button>
                        </div>
                      )}
                      ...
                      {error && (
                        <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-[20px] flex items-center gap-4 text-red-500 shadow-2xl animate-shake">
                           ...
                           <p className="text-[10px] font-black uppercase tracking-widest leading-tight italic">{error}</p>
                        </div>
                      )}
                    </form>
                  )}
            ...
            <div className="w-full max-w-[420px] mt-10">
              {!success && (
                <button 
                  ...
                  className="w-full h-16 bg-slate-900 text-white rounded-[24px] font-black text-[11px] uppercase tracking-[0.4em] shadow-xl hover:bg-green-600 hover:scale-[1.02] active:scale-95 transition-all duration-500 flex items-center justify-center gap-5 disabled:opacity-50 group"
                >
                  {loading ? <Loader2 size={24} className="animate-spin" /> : 
                    <>
                      <span>{view === 'login' ? 'Conectar Nodo' : view === 'register' ? 'Sincronizar Operador' : 'Enviar Reseteo'}</span> 
          ...
          <footer className="py-10 text-center border-t border-white/5 bg-white/[0.01]">
             <p className="text-[8px] font-black text-slate-800 uppercase tracking-[0.4em] italic">
