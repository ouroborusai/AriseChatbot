...
export default function TemplatesPage() {
  ...
  const filteredTemplates = useMemo(() => {
    return templates.filter((t) => {
      ...
      const segMatch = selectedSegment === 'todos' || t.segment === selectedSegment || t.segment === 'todos';
  ...
  }, [templates, selectedCategory, selectedSegment, selectedService, searchQuery]);
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm font-medium text-slate-600">Cargando plantillas...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4 h-full flex flex-col">
      ...
      <div className="card-base">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Flow de Respuestas</h1>
            <p className="text-sm text-slate-500">Visualiza y configura el flujo conversacional de WhatsApp</p>
          </div>
            ...
            <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 ${
                  viewMode === 'flow' ? 'bg-white shadow text-green-600' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition flex items-center gap-2 ${
                  viewMode === 'cards' ? 'bg-white shadow text-green-600' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
        ...
        {stats.byCategory.slice(0, 6).map(cat => (
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border flex items-center gap-2 transition ${
              selectedCategory === cat.id ? 'ring-2 ring-green-400' : 'border-slate-200 hover:border-slate-300'
            }`}
          >
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <input
          ...
          className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm w-48"
        />
        <select
        ...
        {viewMode === 'flow' ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredTemplates.map((template) => {
              ...
              const service = SERVICE_TYPES.find(s => s.id === template.service_type);
              
              return (
                <div
                  key={template.id}
                  onClick={() => handleSelectTemplate(template.id)}
                  className={`card-base cursor-pointer transition hover:shadow-md ${
                    selectedTemplateId === template.id ? 'ring-2 ring-green-400' : ''
                ...
                >
                  ...
                  {template.actions && template.actions.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {template.actions.slice(0, 3).map((a, i) => (
                        <span key={i} className="text-[10px] bg-green-50 text-green-700 px-1.5 py-0.5 rounded">
                          {a.title}
                        </span>
                      ))}
                      {template.actions.length > 3 && (
                        <span className="text-[10px] text-slate-400">+{template.actions.length - 3}</span>
                      )}
                    </div>
                  )}
