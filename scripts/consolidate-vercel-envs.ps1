# consolidate-vercel-envs.ps1
param(
    [string]$Token = "vca_3PWvnZf6D1uEoVQP8k9CgRjRymtDD2Bgk5n4PQuVCXwoipRzFJ2caRk1"
)
...
$listUrl = "https://api.vercel.com/v9/projects/$projectId/env?teamSlug=$teamSlug"

Write-Host "Fetching env list..."
$currentEnvs = (Invoke-RestMethod -Uri $listUrl -Method GET -Headers $headers).envs

foreach ($name in $envVars.Keys) {
    Write-Host "Cleaning $name..."
    $toDelete = $currentEnvs | Where-Object { $_.key -eq $name }
    foreach ($item in $toDelete) {
        $deleteUrl = "https://api.vercel.com/v9/projects/$projectId/env/$($item.id)?teamSlug=$teamSlug"
        Invoke-RestMethod -Uri $deleteUrl -Method DELETE -Headers $headers | Out-Null
    }

    Write-Host "Adding $name (consolidated)..."
    $addUrl = "https://api.vercel.com/v10/projects/$projectId/env?teamSlug=$teamSlug"
    $body = @{
        key    = $name
        value  = $envVars[$name]
        type   = "encrypted"
