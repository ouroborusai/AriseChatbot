...
async function simulateClientChat() {
  ...
  // Obtener datos manualmente para evitar fallos de importación de service
  ...
  const compList = (companies || []).map((c: any) => c.companies);
  ...
  if (context.companies.length > 0) {
    console.log(`- Empresa Activa: ${context.companies.find(c => c.id === activeCompanyId)?.legal_name || 'Ninguna (se pedirá selección)'}`);
  }
  ...
  if (template) {
    ...
    // Simular reemplazo de variables básicas
    content = content.replace('{{name}}', context.contact.name || 'Cliente');
    content = content.replace('{{legal_name}}', context.companies[0]?.legal_name || 'tu empresa');
    content = content.replace('{{rut}}', context.companies[0]?.tax_id || 'RUT no disponible');
    
    console.log(content);
    console.log(`\nBotones asociados:`);
    template.actions?.forEach(a => {
      if (a.type === 'button' || a.type === 'list_item') {
